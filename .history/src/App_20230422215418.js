...
function App() {
    ...
    const renderPlaylists = () => {
        return playlists.map(playlist => (
            <div key={playlist.id}  className = "text-left" onClick={() => handlePlaylistClick(playlist)}>
                {playlist.images.length ? <img className=" rounded-md" width={200} src={playlist.images[0].url} alt=""/> : <div>No Image</div>}
            {playlist.name}
        </div>
    ))
}
